        ...
        try {
            if (type === "sign-up") {
                ...
                if (response.error) {
                    ...
                    // TODO: show error in UI (toast, message, etc.)
                    return
                }

                setUser(response.user)
                router.push("/") // redirect after signup if needed
            }
        ...
        <section className="auth-form">
            <header className='flex flex-col gap-5 md:gap-8'>
                <Link href="/" className="flex cursor-pointer items-center gap-1">
                    <Image src="/icons/logo.svg" width={34} height={34} alt="Financier Logo" />
                    <h1 className="text-26 font-ibm-plex-serif font-bold text-black-1">Financier</h1>
                </Link>
                ...
                <>
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                            ...
                            <Button type="submit" disabled={isLoading} className='w-full bg-blue-700 hover:bg-blue-800 text-white h-11 rounded-lg flex items-center justify-center'>
                                {isLoading ? (
                                    <>
